...
func (b BotInstance) Connect() {
	...
	if b.address != "" {
		...
		if b.err != nil {
			log.Fatalf("Error in BotInstance.Connect()->Connect(): %s\n", b.err)
		}
	} else {
		log.Fatalln("Bot instance was misconfigured when Connect() was called.")
	}
	...
	// Setup the Lua state machine
	L := lua.NewState()
	L.OpenLibs()
	defer L.Close()

	// And register our methods
	L.Register()

	for {
		message, err := b.conn.ReadMessage()
		if err != nil {
			log.Fatalf("Error in BotInstance.Connect()->ReadMessage(): %s\n", err)
		}
...
func (b BotInstance) ShowCommands(event IrcEvent, cmd string) bool {
	dir, err := os.Open(cfg.CommandDir)
	if err != nil {
		return false
	}
	defer dir.Close()
	...
	if cmd == "" {
		...
		for _, fi := range fileInfo {
			if strings.Split(fi.Name(), ".")[0] == cmd {
				...
				cmnd := fmt.Sprintf("%s/%s", cfg.CommandDir, fi.Name())
	...
	for _, fi := range fileInfo {
		if fi.Mode()&0111 != 0 {
			...
			if strings.Split(fi.Name(), ".")[0] == event.command {
				// and now we know the filename matches our command
				cmd := fmt.Sprintf("%s/%s", cfg.CommandDir, fi.Name())
...
func (b BotInstance) BNick(nick string) { b.conn.Send("NICK %s", nick) }

// BotInstance.User(name string)

func (b BotInstance) User(name string)  { b.conn.Send("USER %s * * :...", b.name) }
func (b BotInstance) BUser(name string) { b.conn.Send("USER %s * * :...", b.name) }
